...
export function StepHardwareSelection() {
  ...
  const computeOptions = useMemo(() => {
    ...
    ver.compatibility_matrix.forEach((entry) => {
      ...
      // Filter logic:
      // If NVIDIA GPU detected: Show CUDA options + CPU. Hide ROCm.
      // If AMD GPU detected: Show ROCm options + CPU. Hide CUDA.
      // If No GPU detected: Show CPU only.
      // If Apple Silicon: Show CPU (MPS).
      ...
      // Always show CPU as fallback? Or only if no GPU?
      // User said: "If user is N-card, don't show ROCm. If A-card, don't show CUDA."
      // Usually CPU is always a valid fallback.
  ...
  // Define handleSelectCompute first (before useEffect that uses it)
