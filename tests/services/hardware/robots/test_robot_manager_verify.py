...
def test_motor_bus_verify_tuple_key_and_variant_optional():
    ...
    bus = FakeCanBus()
    mi = MotorModelInfo(
        model="DM4310", model_ids=[17168], limits={}, variant=None, brand=MotorBrand.DAMIAO, encoder_resolution=65536.0
    )
...
def test_motor_bus_verify_mismatch_brand_returns_false():
    ...
    bus = FakeBus()
    mi = MotorModelInfo(
        model="STS3215", model_ids=[1], limits={}, variant="A", brand=MotorBrand.FEETECH, encoder_resolution=4096.0
    )
...
def test_motor_bus_verify_count_mismatch_returns_false():
    ...
    bus = FakeBus()
    mi = MotorModelInfo(
        model="X", model_ids=[0], limits={}, variant=None, brand=MotorBrand.FEETECH, encoder_resolution=4096.0
    )
    bus.motors = {1: (Mock(), mi)}
