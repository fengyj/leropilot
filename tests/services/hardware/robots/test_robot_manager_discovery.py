...
def test_discover_motor_buses_serial(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def fake_scan_feetech(self: object, id_range: object | None = None) -> dict:
        return {
            1: MotorModelInfo(model="X", model_ids=[1], limits={}, brand=MotorBrand.FEETECH, encoder_resolution=4096.0)
        }
...
def test_discover_motor_buses_with_filters(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda self: [PlatformCANInterface(interface="socketcan:can0", manufacturer="CANInc")],
    )
    ...
    def fake_scan_damiao(self: object, id_range: object | None = None) -> dict:
        return {
            (1, 1): MotorModelInfo(
                model="D", model_ids=[2], limits={}, brand=MotorBrand.DAMIAO, encoder_resolution=65536.0
            )
        }
