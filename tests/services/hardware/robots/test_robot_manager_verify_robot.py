...
def test_verify_robot_raises_on_missing_interface() -> None:
    ...
    robot = Robot(
        id="r1",
        name="no-interface",
        motor_bus_connections={"mb": RobotMotorBusConnection(motor_bus_type="Fake", interface=None, baudrate=0)},
...
def test_verify_robot_invalid_raises(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    class FakeBusWithMotor:
        ...
        def scan_motors(self) -> dict:
            ...
            minfo = MotorModelInfo(
                model="M1", model_ids=[1], limits={}, variant=None, brand=MotorBrand.FEETECH, encoder_resolution=4096.0
            )
