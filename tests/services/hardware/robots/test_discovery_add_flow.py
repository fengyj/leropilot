...
def test_discovery_then_add_device(monkeypatch, tmp_path):
    ...
    robots = resp.json()
    assert any(
        r.get("motor_bus_connections", {}).get("motorbus", {}).get("serial_number") == "SN_ADD_1" for r in robots
    )
    ...
    robots2 = resp2.json()
    assert any(
        r.get("motor_bus_connections", {}).get("motorbus", {}).get("serial_number") == "SN_ADD_1" for r in robots2
    )
