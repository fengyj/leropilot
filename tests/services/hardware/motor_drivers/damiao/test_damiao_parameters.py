...
class TestDamiaoParameterAccess:
    ...
    def test_read_parameter_format(self, driver):
        ...
        with (
            patch.object(driver.register, "_send_can_frame") as mock_send,
            patch.object(driver.register, "_recv_motor_response") as mock_recv,
        ):
            mock_send.return_value = True
            # Mock response: data layout per protocol -> bytes[4:8] contain 4-byte little-endian value
            mock_response = Mock()
            # Put uint32 0x00123400 as little-endian bytes [0x00, 0x34, 0x12, 0x00] in positions 4..7
            mock_response.data = bytes([1, 0, 0x33, 0x00, 0x00, 0x34, 0x12, 0x00])
            ...
            # Verify response parsing returns raw 4 bytes
            assert result == bytes([0x00, 0x34, 0x12, 0x00])
            # And that the uint32 helper decodes expected integer
            assert driver.register.read_uint32(id_tuple, param_addr) == 0x00123400
    ...
    def test_write_parameter_format(self, driver):
        ...
        with (
            ...
            mock_send.return_value = True
            # Mock acknowledgment response (4-byte little-endian value bytes appended)
            mock_response = Mock()
            mock_response.data = bytes([2, 0, 0x55, 0x00, 0x78, 0x56, 0x00, 0x00])
            mock_recv.return_value = mock_response

            result = driver.write_parameter(id_tuple, param_addr, value)

            # Verify send was called with correct data to PARAM_ID (value encoded as 4-byte LE)
            expected_data = bytes([
                motor_id & 0xFF,  # motor_id_low
                (motor_id >> 8) & 0xFF,  # motor_id_high
                DamiaoConstants.CAN_CMD_WRITE_PARAM,  # command
    ...
    def test_save_parameters_format(self, driver):
        ...
        with (
            patch.object(driver, "_send_can_frame") as mock_send,
    ...
    def test_write_parameter_send_failure(self, driver):
        ...
        with patch.object(driver.register, "_send_can_frame") as mock_send:
    ...
    def test_identify_model_with_parameter_read(self, driver):
        """identify_model is hardcoded to return generic DM model"""
        motor_id = (1, 1)

        result = driver.identify_model(motor_id)

        assert result is not None
        assert result.model == "DM"

    def test_identify_model_with_float_encoded_read(self, driver):
        """If motor returns float-encoded bytes (e.g., 0x41700000 -> 15.0), identification should handle fallback and not crash."""
        motor_id = (3, 19)
        # Since identify_model is hardcoded to return DM, verify it still logs/returns without raising
        result = driver.identify_model(motor_id)
        assert result is not None
        assert result.model == "DM"
