...
from fastapi import APIRouter, HTTPException, BackgroundTasks, Query, Path as PathParam, Body, Response
...
async def probe_connection(
    ...
    baud_rates: Optional[List[int]] = Body(None, embed=True)
):
    """
    ...
    try:
        ...
        result = motor_service.probe_connection(
            interface=interface,
            probe_baud_list=baud_rates
        )
...
async def update_device(
    ...
    connection_settings: Optional[Dict[str, Any]] = Body(None),
    config: Optional[Dict[str, Any]] = Body(None, description="Detailed configuration (motors, custom)")
):
...
async def get_resource(device_id: str, path: str):
    """
...
async def upload_urdf(device_id: str, file: bytes = Body(..., media_type="application/xml")):
    """
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"URDF processing error: {e}")
    finally:
...
async def get_urdf(device_id: str):
    ...
    if urdf_path.exists():
         try:
            ...
            raise HTTPException(status_code=500, detail=f"Failed to read custom URDF: {e}")
...
async def delete_urdf(device_id: str):
    """Delete custom URDF."""
    manager = get_hardware_manager()
    device = manager.get_device(device_id)
    if not device:
