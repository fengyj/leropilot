...
async def robot_websocket(
    ...
    This handler performs API-level validation and delegates the full
    teleoperation lifecycle to `RobotTelecontrolSession`.
    """
    ...
    session = RobotTelecontrolSession(device_id, normalized=normalized, fps=fps)
    ...
    try:
        ...
        try:
            ...
            logger.exception(f"Error stopping telecontrol session for {device_id}")
