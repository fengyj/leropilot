"""
Lightweight URDF parser, validator, and resource file accessor.
...
- get_robot_resource(model: str, resource_name: str) -> Path | None
- get_urdf_resource(model: str) -> Path | None
    ...
    try:
        ...
        for ln in root.findall("link"):
            ...
            if inertial is not None:
                ...
                if mass_el is not None and "value" in mass_el.attrib:
                    try:
                        mass_val = mass_el.attrib.get("value")
                        mass = float(mass_val) if mass_val is not None else None
                    except Exception:
                        mass = None
                inertia_el = inertial.find("inertia")
                if inertia_el is not None:
                    inertia_val: dict[str, float] = {}
                    bad = False
                    for k in ["ixx", "iyy", "izz", "ixy", "ixz", "iyz"]:
                        v = inertia_el.attrib.get(k)
                        if v is not None:
                            try:
                                inertia_val[k] = float(v)
                            except Exception:
                                bad = True
                                break
                    inertia = None if bad else inertia_val
        ...
        for jn in root.findall("joint"):
            ...
            if limit_el is not None:
                try:
                    lower_s = limit_el.attrib.get("lower")
                    lower = float(lower_s) if lower_s is not None else None
                    upper_s = limit_el.attrib.get("upper")
                    upper = float(upper_s) if upper_s is not None else None
                    effort_s = limit_el.attrib.get("effort")
                    effort = float(effort_s) if effort_s is not None else None
                    velocity_s = limit_el.attrib.get("velocity")
                    velocity = float(velocity_s) if velocity_s is not None else None
                    limits = {
                        "lower": lower,
                        "upper": upper,
                        "effort": effort,
...
def _check_kinematic_chain(parsed_joints: list[dict], parsed_links: list[dict]) -> None:
    ...
    all_links: set[str] = {str(link.get("name")) for link in parsed_links if link.get("name") is not None}
    all_children: set[str] = {str(j.get("child")) for j in parsed_joints if j.get("child") is not None}
    roots: set[str] = all_links - all_children
    if len(roots) != 1:
...
def get_joint_chain(urdf_path: str, from_link: str, to_link: str) -> list[str] | None:
    try:
        ...
        joint_map: dict[str, list[tuple[str, str]]] = {}
        for j in parsed.get("joint_info", []):
            parent = j.get("parent")
            name = j.get("name")
            child = j.get("child")
            if parent is None or name is None or child is None:
                continue
            joint_map.setdefault(parent, []).append((name, child))
...
# ============================================================================
# Robot Resource File Access
# ============================================================================


def get_robot_resource(model: str, resource_name: str) -> Path | None:
    """Get a resource file path for a robot model from built-in resources.

    Args:
        model: Robot definition ID (e.g., "so100-follower-standard")
        resource_name: Resource filename (e.g., "robot.urdf", "meshes/link1.stl")

    Returns:
        Path to the resource file or None if not found

    Example:
        >>> urdf_path = get_robot_resource("so100-follower-standard", "robot.urdf")
        >>> if urdf_path:
        ...     with importlib.resources.as_file(urdf_path) as fp:
        ...         print(fp)
    """
    try:
        ...
        resource_path = builtin_dir.joinpath(resource_name)
        try:
            # Check if the resource exists by attempting to read it
        ...
        logger.debug(f"Resource not found: {model}/{resource_name}")
        return None
    except Exception as e:
        logger.debug(f"Error accessing robot resource: {e}")
        return None


def get_urdf_resource(model: str) -> Path | None:
    """Get the URDF file for a robot model from built-in resources.
    ...
    Example:
        >>> urdf_path = get_urdf_resource("so100-follower-standard")
        >>> if urdf_path:
        ...     result = validate_file(str(urdf_path))
    """
    try:
        resource_files = importlib.resources.files("leropilot.resources")
        builtin_dir = resource_files.joinpath("robots").joinpath(model)

        # Find first .urdf file
        try:
            for p in builtin_dir.iterdir():
                if p.name.endswith(".urdf"):
                    return p
        except Exception:
            pass

        logger.debug(f"URDF not found for model: {model}")
        return None
    except Exception as e:
        logger.debug(f"Error accessing URDF resource: {e}")
        return None
